...
REM ============================================================================
REM inc_build.bat - Increment build version (Z)
REM
REM Usage: inc_build.bat [plugin1] [plugin2] ...
REM        inc_build.bat all
REM        inc_build.bat              (interactive mode)
REM
REM Version format: x.y.z
REM   x = major (unchanged)
REM   y = minor (unchanged)
REM   z = build (incremented)
REM
REM Example: 1.2.5 -> 1.2.6
REM ============================================================================
...
echo.
echo ============================================================================
echo                     Increment Build Version (Z)
echo                     Example: 1.2.5 -^> 1.2.6
echo ============================================================================
...
for %%P in (%PLUGINS_TO_UPDATE%) do (
    ...
    if not exist "!PLUGIN_FILE!" (
        ...
        set /a "NEW_Z=!VZ!+1"
        set "NEW_VER=!VX!.!VY!.!NEW_Z!"

        REM Update the file
        powershell -NoProfile -ExecutionPolicy Bypass -Command "$f='!PLUGIN_FILE!'; $c=Get-Content $f -Raw; $c=$c -replace 'mobase\.VersionInfo\s*\(\s*\d+\s*,\s*\d+\s*,\s*\d+\s*(,\s*\d+\s*)?\)','mobase.VersionInfo(!VX!, !VY!, !NEW_Z!)'; Set-Content $f -Value $c -NoNewline"
...
echo.
echo ============================================================================
echo Build Update Summary: %UPDATE_COUNT% updated, %FAILED_COUNT% failed
echo ============================================================================
