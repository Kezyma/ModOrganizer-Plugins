...
class PluginFinderAutoInstall(PluginFinderPlugin, mobase.IPluginInstallerSimple):
    """ Plugin Finder installer plugin. Handles installing MO2 plugins. """
    ...
    def description(self):
        return self.__tr("Allows automatic install for Mod Organizer 2 plugins.")
    ...
    def findInstallerPathToData(self, path: str, entry: mobase.FileTreeEntry):
        if str(entry.name()).lower() == str(self.pluginfinder.paths.gameDataDir()).lower():
    ...
    def priority(self) -> int: #required
        return self.pluginfinder.settings.priority()
    ...
    def install(self, name: mobase.GuessedString, tree: mobase.IFileTree, version: str, nexus_id: int) -> Union[mobase.InstallResult, mobase.IFileTree, Tuple[mobase.InstallResult, mobase.IFileTree, str, int]]:
        ...
        if res == QtWidgets.QDialog.Accepted:
            ...
            # Get the path to the plugins for MO2.
            pluginPath = self.pluginfinder.paths.modOrganizerPluginPath()
            # Get the path to the zip for the plugin to install.
            # Unzip the plugin to the staging area.
            # If there is a folder with an init.py file, copy it to the plugins folder.
            # If not, copy all py, ini & json files to the plugins folder.
            # Restart MO2.
    ...
    def minRootTypes(self):
        """ The minimum possible file types for a valid plugin install. At least one of these must be present """
        return [
            "py"
            ]

    def excRootFiles(self):
        """ List of files or folders that should never be installed. """
        return [
            ]
    ...
    def getInstallDialog(self):
        ...
        dialog.setWindowTitle("Plugin Finder Install")
